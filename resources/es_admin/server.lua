...
AddEventHandler('es_admin:all', function(type)
	...
	TriggerEvent('f:getPlayer', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			if available and user.getGroup() == "owner" then
...
AddEventHandler('es_admin:quick', function(id, type)
	local Source = source
	TriggerEvent('f:getPlayer', source, function(user)
		TriggerEvent('f:getPlayer', id, function(target)
			TriggerEvent('es:canGroupTarget', user.getGroup(), groupsRequired[type], function(available)
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(target)
					if target and available then
						if user.getGroup() == "superadmin" or user.getGroup() == "owner" then
...
AddEventHandler("login:afterSelection",function(source, user)
		...
		if user.getGroup() == "owner" then
				...
				if(GetPlayerName(USER) == nil)then
					...
					TriggerEvent("es:getAllGroups", function(groups)
						if(groups[GROUP])then
							TriggerEvent("f:setdata", tonumber(USER), "group", GROUP, function(response, success)
								if(true)then
			...
			elseif t == "level" then
				if(GetPlayerName(USER) == nil)then
					...
					if(GROUP ~= nil and GROUP > -1)then
						TriggerEvent("f:setdata", USER, "permission_level", GROUP, function(response, success)
							if(true)then
			...
			elseif t == "money" then
				if(GetPlayerName(USER) == nil)then
					...
					GROUP = tonumber(GROUP)
					if(GROUP ~= nil and GROUP > -1)then
						TriggerEvent('f:getPlayer', USER, function(target)
			...
			TriggerClientEvent('chatMessage', source, 'SYSTEM', {255, 0, 0}, "Owner is required to do this")
		end
	end)	
end)
...
AddEventHandler('rconCommand', function(commandName, args)
	if commandName == 'setadmin' then
		...
		TriggerEvent("f:setdata", tonumber(args[1]), "permission_level", tonumber(args[2]), function(response, success)
			RconPrint(response)
	...
	elseif commandName == 'setgroup' then
		...
		TriggerEvent("es:getAllGroups", function(groups)
			...
			if(groups[args[2]])then
				TriggerClientEvent('es_admin:setGroup', args[1], args[2])
				RconPrint("\nAdded group to player.")
				TriggerEvent("f:setdata", tonumber(args[1]), "group", args[2], function(response, success)
					if(true)then
	...
	elseif commandName == 'setmoney' then
			...
			TriggerEvent("f:getPlayer", tonumber(args[1]), function(user)
				if(user)then
	...
	elseif commandName == 'kickall' then
			if #args < 1 then
					RconPrint("Usage: kickall [reason]\n")
					CancelEvent()
					return
			end
			TriggerClientEvent("kickall", -1, table.concat(args, " "))
			CancelEvent()
	end
end)
...
TriggerEvent('es:addCommand', 'admin', function(source, args, user)
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Level: ^*^2 " .. user.get('permission_level'))
...
TriggerEvent('es:addAdminCommand', 'dv', 1, function(source, args, user) 
     TriggerClientEvent('es_admin:deleteVehicle', source)
...
TriggerEvent('es:addCommand', 'report', function(source, args, user)
	...
	TriggerEvent("es:getPlayers", function(pl)
		for k,v in pairs(pl) do
			TriggerEvent("f:getPlayer", k, function(user)
...
end, {help = "Spawn a model", params = {{name = "model name", help = "The name of the model"}}})
...
function doHashes()
  lines = {}
  for line in io.lines("resources/es_admin/input.txt") do
  	lines[#lines + 1] = line
  end
...
TriggerEvent('es:addGroupCommand', 'pos', "owner", function(source, args, user)
	if args[2] then
		if args[2] == "n" or args[2] == "h" then
			TriggerClientEvent('es_admin:givePosition', source, args[2])
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Please specify what you want! (n = normal,h = +heading)")
		end
	else
		TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Please specify what you want! (n = normal,h = +heading)")
	end
