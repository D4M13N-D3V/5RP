...
local car_speeds = {
    --Supers
    {model="gtr", speed=150},
    {model="lp6504", speed=150},
    {model="488gtb", speed=150},
    {model="911turbos", speed=155},
    {model="amv12", speed=150},
    {model="vc7", speed=150},
    {model="FTypeC", speed=150},
    {model="gallardo", speed=150},
    {model="hellcat", speed=150},
    {model="r8ppi", speed=155},
    {model="rmodm4gts", speed=150},
    --Sports
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if  IsInputDisabled(2) == 1 and IsControlJustPressed(1, 288) and  carshop_menu == false and customs_menu == false and carrental_menu == false and engineon == true then --- K (Open)
...
Citizen.CreateThread(function()
    while true do
		...
		if(IsControlJustPressed(1,47) and IsInputDisabled(2) == 1 )then
            if voip == "~g~Whisper" then
                voip = "~b~Normal"
                NetworkSetTalkerProximity(15.0)
            elseif voip == "~b~Normal" then
                voip = "~r~Shout"
                NetworkSetTalkerProximity(35.0)
            elseif voip == "~r~Shout" then
                voip = "~g~Whisper"
                NetworkSetTalkerProximity(4.0)
            end
		end
        ...
        if NetworkIsPlayerTalking(PlayerId()) then
            drawThisTxt(0.287, 0.965, 0.25, 0.03, 0.50,"~g~You are talking ~w~| Voice : "..voip,255,255,255,255,6)
        else
            drawThisTxt(0.286, 0.965, 0.25, 0.03, 0.50,"Voice : "..voip,255,255,255,255,6)
        end
    end
end)
--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--
