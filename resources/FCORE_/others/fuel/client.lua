...
function burnFuel(drf_veh)
  if engineon then
    ...
    local speedMph = math.ceil(speed * 1.5)
        ...
        if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
            ...
            if(fuelPumpNear(veh)) then
                if veh and GetPedInVehicleSeat(veh, -1) == playerPed then
                    ...
                    if IsControlJustReleased(1, 76) then
						engineon = true
						hold = false
						TriggerEvent('fuel:stopFuelbeingAdded')
                    ...
                    if fuelcount < 100000 then
	                    if not hold then
	                    	drawTxt("Hold ~g~SPACE~w~ to refuel!",0,1,0.5,0.8,0.6,255,255,255,255)
	                    end
