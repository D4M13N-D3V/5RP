...
AddEventHandler('sendSessionPlayerNumber', function(clientPlayerNumber, name, id)
	...
	if(clientPlayerNumber < serverPlayerNumber) then
		if(clientPlayerNumber == 1) then -- if player are solo
			...
			TriggerClientEvent('chatMessage', -1, 'SERVER', { 0, 0x99, 255 },  "^2"..name .. " ^0(^4" ..id .. "^0) ^Were Kicked : ^2" .. reason) -- In game chat message (example: John Doe (ID) was kicked for Auto-Kick Session solo détectée)
