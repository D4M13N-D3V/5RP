...
local TEXT = {
    ...
    InfotaxiNoAppel = '~g~No call waiting',
    InfotaxiNbAppel = '~w~ Call waiting',
...
Citizen.CreateThread(function()
    while true do
        ...
        if isTaxi then
            ...
            if inService then
                inamount = 0.03
                drawTaxiTxt(0.286, 0.935, 0.25, 0.03, 0.50,taxi_nbMissionEnAttenteText,255,255,255,255,6)
            end
        end

        if taxi_call_accept ~= 0 then
            if taxi_call_accept == 1 then
                drawTaxiTxt(0.286, 0.905 - inamount, 0.25, 0.03, 0.50,TEXT.CALL_INFO_OK,255,255,255,255,6)
            else 
                if taxi_nbtaxiInService == 0 then
                    drawTaxiTxt(0.286, 0.905 - inamount, 0.25, 0.03, 0.50,TEXT.CALL_INFO_NO_PERSONNEL,255,255,255,255,6)
                elseif taxi_nbtaxiDispo == 0 then
                    drawTaxiTxt(0.286, 0.905 - inamount, 0.25, 0.03, 0.50,TEXT.CALL_INFO_ALL_BUSY,255,255,255,255,6)
                else
                    drawTaxiTxt(0.286, 0.905 - inamount, 0.25, 0.03, 0.50,TEXT.CALL_INFO_WAIT,255,255,255,255,6)
                end
            end 
        end
    end
end)
function drawTaxiTxt(x,y ,width,height,scale, text, r,g,b,a,font)
    SetTextFont(font)
