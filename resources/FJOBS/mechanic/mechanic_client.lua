...
local TEXT = {
    ...
    InfoMechanicNoAppel = '~g~No Call Waiting',
    InfoMechanicNbAppel = '~w~ Call Waiting',
    ...
    for _, pos in pairs(coords) do 
        if pos.type == type then
            ...
            if vehi == 0 then
                ...
                SetVehicleNumberPlateText(myVehiculeEntity, "TOW001")
...
local function CustomVehicleDommage()
    ...
    if vehicle ~= 0 then 
        if not IsPedInAnyHeli(GetPlayerPed(-1)) then
            ...
            if vehicleHealth <= 0 or (engineHealth <= 900 and engineHealth > 151) or petrolTankeHealth <= 900  or vehicleHealth - 2000 + engineHealth + petrolTankeHealth <= 0 then   
                SetVehicleEngineHealth(vehicle, -1.0)
                SetVehicleBodyHealth(0.0)
                SetVehicleUndriveable(vehicle, true)
            elseif (2000 - engineHealth - petrolTankeHealth) >= 65 and engineHealth > 150 then
                SetVehicleDoorOpen(vehicle, 4, 0, 0)
...
function drawMechTxt(x,y ,width,height,scale, text, r,g,b,a,font)
    SetTextFont(font)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isMechanic then
            ...
            if inService then
                inamount = 0.03
                drawMechTxt(0.286, 0.935, 0.25, 0.03, 0.50,mechanic_nbMissionEnAttenteText,255,255,255,255,6)
            end
        end
        ...
        if mechanic_call_accept ~= 0 then
            if mechanic_call_accept == 1 then
                drawMechTxt(0.286, 0.935 - inamount, 0.25, 0.03, 0.50,TEXT.CALL_INFO_OK,255,255,255,255,6)
            else 
                if mechanic_nbMechanicInService == 0 then
                    drawMechTxt(0.286, 0.935 - inamount, 0.25, 0.03, 0.50,TEXT.CALL_INFO_NO_PERSONNEL,255,255,255,255,6)
                elseif mechanic_nbMechanicDispo == 0 then
                    drawMechTxt(0.286, 0.935 - inamount, 0.25, 0.03, 0.50,TEXT.CALL_INFO_ALL_BUSY,255,255,255,255,6)
                else
                    drawMechTxt(0.286, 0.935 - inamount, 0.25, 0.03, 0.50,TEXT.CALL_INFO_WAIT,255,255,255,255,6)
                end
            end  
        end
    end
...
function acceptMission(data) 
    ...
    TriggerServerEvent('mechanic:AcceptMission', mission.id)
...
function updateMenuMission() 
    ...
    for _,m in pairs(listMissions) do 
        ...
        if #m.acceptBy ~= 0 then
            item.Title = item.Title .. ' (current mission)'
      ...
      --           find = true
       --      end
        ...
        if nbMissionEnAttente == 0 then 
             mechanic_nbMissionEnAttenteText = TEXT.InfoMechanicNoAppel
         else
            mechanic_nbMissionEnAttenteText = '~g~ ' .. nbMissionEnAttente .. ' ' .. TEXT.InfoMechanicNbAppel
         end
    --     Citizen.Trace('ok')
    --     if not find then
    --         currentMissions = nil
