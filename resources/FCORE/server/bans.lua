			    ...
			    if age then
					...
					for k,v in pairs(bantable["Anticheat"]) do
						if v.identifier == identifier or v.license == license then
							...
							banreason = identifier.."("..name.."), you are banned! Reason: "..v.reason.." - You can appeal if you wish."
							break
						end
					end
					if acban then
						...
						for k,v in pairs(bantable["Normal"]) do
							if v.identifier == identifier or v.license == license then
								...
								if currentTime > v.duration then
									...
									nban = true
									banreason = identifier.."("..name.."), you are banned! Expires: "..showdate.month.."/"..showdate.day.."/"..showdate.year.." at "..showdate.hour..":"..showdate.min..":"..showdate.sec.." Reason: "..v.reason.." - You can appeal if you wish"
									break
								end
							end
						end
						if nban then
							...
							if whitelist_active then
								if whitelist[identifier] then
									...
									setReason(name.." you must be whitelisted to play on this server, join one of our public servers...")
									CancelEvent()
								end
							else
								print("[BANS]::"..name.." has cleared ban check for everything else")
...
function BanCheater(reason,source)
	...
	log("bans", GetPlayerName(source).." | "..identifier.." | "..license.." | Forever | "..reason.." | Anti-Cheat")
			PerformHttpRequest('https://discordapp.com/api/webhooks/391447885721632771/wvhD824vG77U4n5XsJT6GILEBEGIdkQ5FWc0G304MvOpJO4Eqzkf0d8fuf0xJgm4_ScD', function(err, text, headers) end, 'POST', json.encode(
			{
...
function BanUser(target,time,reason,source)
	...
	if identifier ~= nil then
		if license ~= nil then
			...
			DropPlayer(target, reason.." - You can appeal if you wish")
		else
