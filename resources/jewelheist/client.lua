...
function createDriver()
	...
	HeistTruckDriver = CreatePedInsideVehicle(HeistTruck, 4, hash, -1, true, false)
...
function startPartTwo()
	...
	Citizen.CreateThread(function()
		...
		while PartTwo == true and startTime + endTime >= GetGameTimer() and HeistActive do 
			...
			if startTime + 30000 < GetGameTimer() then
				if GetPlayerFromServerId(Heistleader) == PlayerId() then
			...
			if not guardsRemoved then
				for i=1, #Weapons do
					if GetCurrentPedWeapon(GetPlayerPed(-1), GetHashKey(Weapons[i]), 1) then
						hasWeapon = true
					end
				end
			end
...
AddEventHandler("jewel-heist:start", function(carID, current_robbers)
	...
	Citizen.CreateThread(function()
		...
		if startTime + endTime < GetGameTimer() then
			...
			SetVehicleNumberPlateText(HeistVehicle, "JEWEL0"..carID)
