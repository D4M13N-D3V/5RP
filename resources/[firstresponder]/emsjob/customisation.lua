        	...
        	if(isInService) then
		        ...
		        for k,v in ipairs(paramedic_clothing_emplacement) do
		            if(Vdist(pos.x, pos.y, pos.z, v.x, v.y, v.z) < 15.0)then
		            	DrawMarker(27, v.x, v.y, v.z-1, 0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 0, 155, 0, 155, 0, 0, 2, 0, 0, 0, 0)
...
function ChangeToSkin(skin)
	...
	if changemodel then
		...
		if IsModelInCdimage(model) and IsModelValid(model) then
			...
			GiveWeaponToPed(GetPlayerPed(-1), GetHashKey("WEAPON_FLARE"), 20, true)
			GiveWeaponToPed(GetPlayerPed(-1), GetHashKey("WEAPON_FIREEXTINGUISHER"), 2000, true)
		else
