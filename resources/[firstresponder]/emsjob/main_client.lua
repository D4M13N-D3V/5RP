...
local takingService = {
	{x=1155.26, y=-1520.82, z=34.9},
	{x=295.411, y=-1446.88, z=30.5},
	{x=361.145, y=-584.414, z=29.2},
	{x=-449.639, y=-340.586, z=34.8},
	{x=-874.467, y=-307.896, z=39.8},
	{x=-677.135, y=310.275, z=83.5},
	{x=1839.39, y=3672.78, z=34.6},
	{x=-242.968, y=6326.29, z=32.8},
}
local inamount = 0
...
function isNearTakeService()
	for i = 1, #takingService do
		...
		if(distance < 30) then
			DrawMarker(27, takingService[i].x, takingService[i].y, takingService[i].z-1, 0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.0, 0, 155, 0, 155, 0, 0, 2, 0, 0, 0, 0)
		end
		if(distance < 2) then
			return true
		end
    ...
    while true do
        ...
        if MedicCallStatus ~= 0 then
            if MedicCallStatus == 1 then
                drawThisTxt(0.286, 0.905 - inamount, 0.25, 0.03, 0.50,'~g~A unit will arrive at the location of the call',255,255,255,255,6)
            else 
                if activeMedics == 0 then
                    drawThisTxt(0.286, 0.905 - inamount, 0.25, 0.03, 0.50,'~r~No paramedics in service',255,255,255,255,6)
                elseif availableMedics == 0 then
                    drawThisTxt(0.286, 0.905 - inamount, 0.25, 0.03, 0.50,'~o~All ambulances are occupied',255,255,255,255,6)
                else
                    drawThisTxt(0.286, 0.905 - inamount, 0.25, 0.03, 0.50,'~b~Your call is on hold',255,255,255,255,6)
                end
            end 
        end
    end
