local stationGarage = {
    {x=-237.292, y=6332.39, z=32.8},
    {x=1842.64, y=3667.10, z=33.9},
    {x=-657.582, y=293.92, z=81.9},
    {x=-876.029, y=-300.418, z=39.9},
    {x=1169.01, y=-1509.82, z=34.9},
    {x=303.086, y=-1439.04, z=30.3},
    {x=364.135, y=-591.145, z=29.1},
    {x=-475.254, y=-352.322, z=34.8},
}

local stationHeliGarage = {
    {x=-475.22604370117, y=5988.6201171875, z=31.9},
    {x=313.30563354492, y=-1465.1845703125, z=46.8},
    {x=299.50131225586, y=-1453.5455322266, z=46.8},
    {x=352.01733398438, y=-588.12835693359, z=74.6},
    {x=1770.2397460938, y=3239.8703613281, z=42.5},
    {x=1216.7823486328, y=-1516.5510253906, z=34.8},
}
...
if settings.force == "LifeMed" then
	--LifeMed
	cars = {
        {"Fire Truck", {model="firetruk",type=""},{"ems probationary", "ems paramedic", "ems doctor", "ems specialist", "ems lieutenant", "ems captain", "ems asst. chief", "ems chief"}},
        {"Ambulance", {model="ambulance",type=""},{"ems probationary", "ems paramedic", "ems doctor", "ems specialist", "ems lieutenant", "ems captain", "ems asst. chief", "ems chief"}},
        {"Tahoe", {model="ems1",type=""},{"ems specialist", "ems lieutenant", "ems captain", "ems asst. chief", "ems chief"}},
		{"Lifeguard Granger", {model="lguard", type=""},{"ems asst. chief"}}
    }
...
function isNearStationGarage()
    for i = 1, #stationGarage do
        ...
        if(distance < 30) then
            DrawMarker(27, stationGarage[i].x, stationGarage[i].y, stationGarage[i].z-1, 0, 0, 0, 0, 0, 0, 2.0, 2.0, 1.0, 0, 155, 0, 155, 0, 0, 2, 0, 0, 0, 0)
            if(distance < 2) then
                return true
...
function isNearStationHeliGarage()
    for i = 1, #stationHeliGarage do
        ...
        if(distance < 30) then
            DrawMarker(27, stationHeliGarage[i].x, stationHeliGarage[i].y, stationHeliGarage[i].z-1, 0, 0, 0, 0, 0, 0, 9.0, 9.0, 1.0, 0, 155, 0, 155, 0, 0, 2, 0, 0, 0, 0)
            if(distance < 7) then
                return true
