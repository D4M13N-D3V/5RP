...
RegisterNetEvent('police:heal')
AddEventHandler('police:heal', function()
...
RegisterNetEvent('police:cuffAnimation')
AddEventHandler( 'police:cuffAnimation', function(cop)

	TaskPlayAnim(GetPlayerPed(GetPlayerFromServerId(cop)),"mp_arresting","a_uncuff",1.0,-1.0, 3000, 0, 1, true, true, true)
end)
...
AddEventHandler('police:getArrested', function()
	if(isInService == false and drag == false) then
		...
		if(handCuffed) then
			...
			exports.pNotify:SendNotificartion({text = "You have been handcuffed.",type = "error",timeout = 3000,layout = "centerRight",queue = "left"})
		else
