    ...
    while true do
        ...
        if(isCop) then
            if(isInService) then
				...
				if IsControlJustPressed(1,311) and IsInputDisabled(2) == 1 then
		        ...
		        if ( IsInputDisabled(2) == 1 and IsControlJustPressed(1, 289) and IsEntityDead(GetPlayerPed(-1))==false )then --- LEFTALT (Open)
    ...
    Menu.addOption("cop_menu", function()
        if(Menu.Option("Revive"))then
        	if GetEntityHealth(GetPlayerPed(-1)) == 0 then
            	TriggerEvent("police:heal", PlayerId())
            else
				local t, distance = GetClosestPlayer()
				if(distance ~= -1 and distance < 3) then
	                if GetEntityHealth(GetPlayerPed(t)) < 10 then
	                    ...
	                    TriggerServerEvent("police:revive", GetPlayerServerId(t))
	                end
				else
					Messages(1)
				end
...
function Vehicle()
    ...
    Menu.addOption("cop_menu_vehicle", function()
        if(Menu.Option("Impound vehicle"))then
			local pos = GetEntityCoords(GetPlayerPed(-1), true)
			local closestVeh = GetClosestVehicle(pos.x, pos.y, pos.z, 5.000, 0, 70)
			if DoesEntityExist(closestVeh) then
				if exports.FCORE_:GetActiveTows() > 0 then
					DecorSetBool(closestVeh, "Impounded", true)
					local var1, var2 = GetStreetNameAtCoord(pos.x, pos.y, pos.z, Citizen.ResultAsInteger(), Citizen.ResultAsInteger())
					if GetVehicleNumberPlateText(closestVeh) then
						local vehicle_coord = GetEntityCoords(closestVeh, true)
						local vehicle_coords = {vehicle_coord.x,vehicle_coord.y,vehicle_coord.z}
						TriggerServerEvent("tow:addBlip", vehicle_coords, GetVehicleNumberPlateText(closestVeh),tostring(GetStreetNameFromHashKey(var1)))
					else
						TriggerEvent("customNotification", "Vehicle impounded.")
	                ...
	                TriggerEvent("customNotification", "Vehicle impounded.")
	                if GetVehicleNumberPlateText(closestVeh) then
	                	TriggerServerEvent("police:impound", GetVehicleNumberPlateText(closestVeh), closestVeh)
	                end
					SetEntityAsMissionEntity(closestVeh, true, true)
					Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(closestVeh))
				end
			else
				Notify("No car found nearby!")
			end
        end
    end)
end

RegisterNetEvent("police:deleteLocalCar")
AddEventHandler("police:deleteLocalCar", function(car)
		...
		if(Menu.StringArray("Fine:", {"< Optional >", "< Not Optional >"}, finepos,function(cb) finepos = cb end)) then
			...
			if(distance ~= -1 and distance < 3) then
				...
				if(amount == -1) then
		            ...
		            local finefill = AutoFill()
		            DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", finefill.cost , "", "", "", 1000)
        ...
        if(Menu.Option("Jail"))then
		    ...
		    if(distance ~= -1 and distance < 3) then
		        ...
		        if(time == "xxsdrtghyuujhdjsjenenfjfjtjtjtj") then
		            ...
		            local jailtimefill = AutoFill()
		            DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", jailtimefill.time , "", "", "", 1000)
		            while (UpdateOnscreenKeyboard() == 0) do
		                  DisableAllControlActions(0);
		                Wait(0);
		            end
...
local charges = {
    {title="Felonies", items = {
        ...
        {charge="Rioting 1st Degree - Inciting", time=450, cost=5000},
        ...
        {charge="Money Laundering", time=900, cost=5000},
    }},
    {title="Misdemeanors", items = {
        {charge="Attempted Grand Theft Auto", time=225, cost=2500},
        ...
        {charge="Criminal Mischief", time=240, cost=2000},
    }},
    {title="Traffic Violations", items = {
        {charge="Failure to Stop - Red Light", time=0, cost=500},
        {charge="Failure to Stop - Stop Sign", time=0, cost=500},
        {charge="No insurance/Expired Registration", time=0, cost=500},
        {charge="Speeding 10-29mph Over", time=0, cost=500},
...
function JailReason(target,jailtime)
    ...
    if(reason == "xxsdrtghyuujhdjsjenenfjfjtjtjtj") then
        ...
        local charges = AutoFill()
        DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", charges.charges.. " - "..GetPlayerName(PlayerId()), "", "", "", 1000)
        while (UpdateOnscreenKeyboard() == 0) do
              DisableAllControlActions(0);
            Wait(0);
        end
